...
export const metadata = helpMetadata({
  ...
  description:
    'Combine themes and one-click style presets to make a livediagram look polished in seconds.',
...
A clean, consistent look does not have to mean fiddling with colours one element at a time. Pair a [theme](/help/canvas/themes/) for the whole tab with [style presets](/help/palette/shapes/style-presets/) on the elements that matter, and a rough diagram becomes a polished one in seconds.
...
<Figure caption="Pick a theme to recolour the whole tab at once, then new elements arrive on-brand.">
  <ThemePicker />
</Figure>
...
<Tip>
  Theme the tab first so new elements arrive on-brand, then use colour and border presets to lift
