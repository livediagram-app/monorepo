...
import { CrossTabLink } from '@/components/illustrations/tabs';
...
export const metadata = helpMetadata({
  ...
  path: '/help/tabs/linking-tabs/',
});

<ArticleLayout title="Linking Across Tabs" description="Turn any element into a jump point: link it to another tab, a specific element on that tab, another diagram, or an external address." categoryTitle="Tabs" categorySlug="tabs">
...
<Figure caption="A linked element on one tab carries a Follow link badge that jumps to its target tab when clicked.">
  <CrossTabLink />
</Figure>
