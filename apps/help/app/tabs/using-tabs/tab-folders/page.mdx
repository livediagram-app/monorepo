...
export const metadata = helpMetadata({
  ...
  path: '/help/tabs/using-tabs/tab-folders/',
});

<ArticleLayout title="Tab Folders" description="Once a diagram grows past a handful of tabs, group the related ones under a named, collapsible folder along the tab bar." categoryTitle="Tabs" categorySlug="tabs" parentArticle={{ title: "Tabs and Tab Folders", slug: "using-tabs" }} subArticles={getSubArticles("using-tabs").filter((a) => a.slug !== "tab-folders")}>
