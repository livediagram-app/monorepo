...
export const metadata = helpMetadata({
  ...
  description:
    'Link an element to another tab, or to a specific element on it, so a diagram with many tabs becomes navigable like a small site.',
  path: '/help/tabs/using-tabs/linking-tabs/',
});

<ArticleLayout title="Linking Across Tabs" description="Turn any element into a jump point: link it to another tab, a specific element on that tab, another diagram, or an external address." categoryTitle="Tabs" categorySlug="tabs" parentArticle={{ title: "Tabs and Tab Folders", slug: "using-tabs" }} subArticles={getSubArticles("using-tabs").filter((a) => a.slug !== "linking-tabs")}>
...
<Tip>
  For an external address, only http, https, and mailto links are allowed. See [Links and Link
  Cards](/help/canvas/links/) for more on linking elements to the web.
</Tip>
