...
Starting or pausing a timer, and starting, ending, or revealing a vote, each leave a single line in the [Activity history](/help/collaboration/history/) so you can see the shape of the session afterwards. The individual dot casts, which happen fast and often, are deliberately not logged, and neither tool adds to Undo: starting a timer or placing a dot is not the kind of thing you would want to Ctrl+Z.
