...
export const metadata = helpMetadata({ title: "Guest vs Account", description: "The livediagram canvas always works without signing in. Learn what you get as a guest and what an account adds.", path: "/help/getting-started/guest-vs-account/" });
...
- **[Teams](/help/features/teams/).** Join or create a team to work in a shared space with teammates.
...
<Note>livediagram is open source, and accounts are optional. A self-hosted deployment can run in pure-guest mode with no sign-in at all, and the editor stays fully usable.</Note>
