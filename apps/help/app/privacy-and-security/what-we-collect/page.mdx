...
export const metadata = helpMetadata({ title: "What We Collect", description: "livediagram records anonymous, first-party, opt-out product events, published openly on a public dashboard. Here is exactly what that means.", path: "/help/privacy-and-security/what-we-collect/" });
...
<Step n={3}>From that point on, your browser sends nothing, regardless of the deployment's setting.</Step>
...
<Note>Telemetry is **off by default**. On a self-hosted deployment it stays completely off unless the operator explicitly enables it, so a fork or private instance emits and serves nothing.</Note>
