...
export const metadata = helpMetadata({ title: "Data Privacy", description: "Where your livediagram diagrams live, what is stored, and the control you have over your own data.", path: "/help/privacy-and-security/data-privacy/" });
...
A diagram is owned by an identity. That identity is either a signed-in account or, for guests, a per-browser id kept in your browser's local storage. Only the owner, people holding a valid [share link](/help/features/sharing/), and (in future) team members can read a diagram. Requests for a diagram you cannot access return a plain "not found", so the API never even confirms a private diagram exists.

The [activity log](/help/features/activity-and-history/) keeps a per-diagram record of editorial changes with author attribution, and older entries are swept automatically after 90 days.

## Staying in control

- **Guest by default.** You can build and share diagrams without ever creating an account. See [working as a guest](/help/getting-started/working-as-a-guest/).
...
<Note>livediagram does not claim end-to-end encryption. Diagram content is protected by authentication and access controls, not by client-side encryption.</Note>
