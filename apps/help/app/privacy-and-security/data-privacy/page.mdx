...
import { DiagramStorage, AccessControl } from '@/components/illustrations/privacy';
...
<Figure caption="Your edits travel through one API to Cloudflare storage, never straight from the browser to the database.">
  <DiagramStorage />
</Figure>
...
<Figure caption="A diagram is tied to its owner, so a request from anyone else comes back as a plain not found.">
  <AccessControl />
</Figure>

The [activity log](/help/activity-panel/what-it-is/) keeps a per-diagram record of editorial changes with author attribution, and older entries are swept automatically after 90 days.
