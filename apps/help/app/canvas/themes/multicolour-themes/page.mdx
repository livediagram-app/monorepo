...
import { MulticolourTheme } from '@/components/illustrations/canvas';
...
export const metadata = helpMetadata({
  ...
  path: '/help/canvas/themes/multicolour-themes/',
});
...
<Figure caption="A multi-colour theme gives the neutral root a trunk colour and tints each branch its own hue.">
  <MulticolourTheme />
</Figure>
...
<Tip>
  Theme cards show a row of small stripes for multi-colour themes and a single dot for single-colour
  ones, so you can spot the rainbow themes in the picker at a glance.
</Tip>
