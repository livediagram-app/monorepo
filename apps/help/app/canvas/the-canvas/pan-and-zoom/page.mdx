...
export const metadata = helpMetadata({
  ...
  path: '/help/canvas/the-canvas/pan-and-zoom/',
});
...
<ArticleLayout
  ...
  subArticles={getSubArticles("the-canvas").filter((a) => a.slug !== "pan-and-zoom")}
>
...
<Figure caption="Drag empty canvas with the grabbing hand to pan, and use the on-screen zoom controls to scale the view.">
  <PanAndZoom />
</Figure>
...
- **Laser** emits a glowing trail that follows your pointer and fades after about a second, shown live to others in your colour.
...
<Tip>
  Spotlight is a view-only aid and never changes your diagram: while it is active the canvas is
  pointer-inert, so clicks cannot accidentally select or move anything.
</Tip>
