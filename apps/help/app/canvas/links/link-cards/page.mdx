...
import { LinkCard } from '@/components/illustrations/canvas';

export const metadata = helpMetadata({
  title: 'Link Cards',
  description:
    "A link card turns a URL into a rich bookmark on the canvas, with the page's title, favicon, site name, and preview image fetched automatically.",
  path: '/help/canvas/links/link-cards/',
});

<ArticleLayout title="Link Cards" description="Bookmark a web page on the canvas: paste a URL and the card fills in with the page's title, favicon, site name, and preview image." categoryTitle="Canvas" categorySlug="canvas" parentArticle={{ title: "Links and Link Cards", slug: "links" }} subArticles={getSubArticles("links").filter((a) => a.slug !== "link-cards")}>
...
<Figure caption="A filled link card showing the page's favicon, title, and site name as a tidy on-canvas bookmark.">
  <LinkCard />
</Figure>
...
<Step n={3}>
  Paste a URL and confirm. The card fetches the preview and fills itself in. To change the URL
  later, double-click the card again.
</Step>
...
<Tip>
  If a card shows only the bare URL, the page may not publish preview metadata. The link still
  works, the card just has less to show.
</Tip>
