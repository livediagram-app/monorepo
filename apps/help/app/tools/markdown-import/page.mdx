...
import { MarkdownToTree } from '@/components/illustrations/tools';

export const metadata = helpMetadata({
  title: 'Markdown Import',
  description:
    'Turn a Markdown outline, such as an XMind or Obsidian export, into a themed tree diagram on a tab.',
  path: '/help/tools/markdown-import/',
});

<ArticleLayout title="Markdown Import" description="Bring a Markdown outline into livediagram as a tidy, themed tree diagram." categoryTitle="Tools" categorySlug="tools" subArticles={getSubArticles("markdown-import")}>
...
<Figure caption="A Markdown outline becomes a tidy left-to-right tree, themed and connected with curved arrows.">
  <MarkdownToTree />
</Figure>
...
<Step n={1}>
  Open the **Import…** action from a tab's ellipsis (…) menu. The Import dialog opens, mirroring the
  Export dialog.
</Step>

<Step n={2}>
  Choose **Markdown** as the format. This sets the file picker's filter and tells livediagram to
  build a tree.
</Step>
...
<Tip>
  Importing a `.json` tab export is also offered in the same dialog, restored exactly as it was
  saved. Pick the format that matches your file: livediagram does not guess.
</Tip>
