...
import { AutoAlignGrid } from '@/components/illustrations/tools';
...
<Figure caption="Shapes that sit a few pixels off the grid snap onto the nearest grid lines.">
  <AutoAlignGrid />
</Figure>
...
- **Running it again changes nothing**: a second pass leaves everything where it is, because it is already on the grid.
...
<Tip>
  Auto-Align is safe to run often. Since running it again changes nothing and it never moves anything far, it's a
