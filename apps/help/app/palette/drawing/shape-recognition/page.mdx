...
export const metadata = helpMetadata({
  ...
  path: '/help/palette/drawing/shape-recognition/',
});
...
Shape recognition is an opt-in mode for the [Pencil tool](/help/palette/drawing/). With it on, a rough sketch is matched against real shapes when you release, so a wobbly square becomes a proper square and a quick loop becomes a circle, no need to draw precisely.
