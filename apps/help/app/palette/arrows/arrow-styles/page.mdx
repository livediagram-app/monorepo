...
import { ArrowStyles } from '@/components/illustrations/palette-shapes-arrows';
...
export const metadata = helpMetadata({
  ...
  path: '/help/palette/arrows/arrow-styles/',
});
...
<ArticleLayout
  ...
  subArticles={getSubArticles("arrows").filter((a) => a.slug !== "arrow-styles")}
>
...
- **Line style** picks the geometry: **Straight** (a single line), **Curved** (a gentle bezier bow), or **Angled** (an axis-aligned L-shaped elbow). Curved and angled lines expose a draggable handle so you can shape the path by hand, covered in [Curve and elbow handles](/help/palette/arrows/curve-and-elbow-handles/).
- **Line thickness** snaps to four presets (Thin, Medium, Thick, Extra-thick).
- **Line pattern** sets the stroke to Solid, Dashed, or Dotted.

<Figure caption="The same connector shown straight, curved, and angled.">
  <ArrowStyles />
</Figure>
