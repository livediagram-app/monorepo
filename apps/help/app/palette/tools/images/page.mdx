...
import { ImageElement, ImagePicker } from '@/components/illustrations/palette-tools';

export const metadata = helpMetadata({
  title: 'Images',
  description:
    'Drop an image onto the canvas, upload the file once, and reuse anything you have uploaded before from your personal gallery.',
  path: '/help/palette/tools/images/',
});

<ArticleLayout title="Images" description="Add screenshots, photos, and mockups to a diagram. Upload a file once, then reuse it across diagrams from your personal gallery." categoryTitle="Palette" categorySlug="palette" parentArticle={{ title: "Tools", slug: "tools" }} subArticles={getSubArticles("images")}>
...
<Step n={1}>
  Pick the Image tool from the palette, then tap the canvas to drop a default-size image, or drag to
  size it as you place it.
</Step>
...
<Step n={3}>
  Drag a file in, click to browse, or paste from your clipboard. The image attaches and the
  placeholder becomes the picture.
</Step>
...
<Figure caption="A placed image element behaves like any boxed element, with selection handles to resize it.">
  <ImageElement />
</Figure>

## Accepted formats

The picker accepts the formats that display reliably in every browser:
...
<Figure caption="The image picker's Gallery tab shows everything you have uploaded, ready to reuse.">
  <ImagePicker />
</Figure>
...
- **Unused uploads are cleared automatically.** Once a day, livediagram removes images that are both more than 30 days old and used in no diagram, so abandoned uploads do not pile up. An image any diagram still uses is kept no matter its age, and recent uploads are left alone until the 30-day window passes.

<Note>
  Image uploads rely on storage that a self-hosted deployment may not have configured. When uploads
  are not available the Image tool is hidden and the picker explains that uploads are not enabled on
  that deployment.
</Note>

<Tip>
  Before deleting from the gallery, open the "Used in N diagrams" badge to see exactly where an
  image is referenced. It saves you from breaking a picture you forgot was in use.
</Tip>
