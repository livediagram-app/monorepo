...
Two more categories tune the display. The **Chart** category has a **Legend** placement picker — **Off**, **Top**, **Left**, **Right**, or **Below** — that shows the label key and chooses which side of the chart it sits on (the **Right** by default); **Off** hides it. The **Animation** category carries chart-specific animations (None / Grow / Pop / Spin / Pulse) with a **Speed** row and a **Repeat** toggle; Grow and Pop play once as an entrance, while Spin and Pulse loop. Hovering a slice always shows a tooltip with its label and value, whether or not the legend is on.
