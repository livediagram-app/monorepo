...
<Figure caption="The same value as a bar that fills from the left and a ring that draws round from the top.">
  <ProgressBarAndRing />
</Figure>
...
<Step n={2}>
  Pick **Progress** for the bar or **Donut** for the ring. It drops onto the canvas already set to
  50% and animates its fill in on arrival.
</Step>

<Step n={3}>
  Resize, move, or recolour it like any other shape. The ring keeps its circular shape as you
  resize.
</Step>
...
<Tip>
  Animations are reduced-motion-safe and freeze on export, so the still image shows the filled level
  rather than a half-drawn frame.
</Tip>
