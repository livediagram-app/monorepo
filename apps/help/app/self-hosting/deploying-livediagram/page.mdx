...
import { RequestRouting, DeployFlow } from '@/components/illustrations/self-hosting';

export const metadata = helpMetadata({
  title: 'Deploying livediagram',
  description:
    'How livediagram is structured as a set of Cloudflare Workers and deployed via GitHub Actions. A conceptual overview for self-hosters.',
  path: '/help/self-hosting/deploying-livediagram/',
});
...
<Figure caption="The router forwards each request to an app by matching the URL path.">
  <RequestRouting />
</Figure>
...
<Figure caption="GitHub Actions deploys the four apps in parallel, then the router last because its bindings depend on them.">
  <DeployFlow />
</Figure>
...
<Tip>
  Next, see [configuration and optional auth](/help/self-hosting/configuration/) for the environment
  variables that turn optional features on or off.
</Tip>
