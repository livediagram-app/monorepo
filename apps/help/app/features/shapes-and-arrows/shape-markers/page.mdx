...
export const metadata = helpMetadata({
  ...
  path: "/help/features/shapes-and-arrows/shape-markers/",
});
...
<ArticleLayout
  ...
  subArticles={getSubArticles("shapes-and-arrows").filter((a) => a.slug !== "shape-markers")}
>
...
<Step n={2}>Open the **Markers** category (it sits in its own band between Border and Collaborate).</Step>
...
<Tip>Use a row of shapes with green, amber, and red dots as a quick status board, then flip a dot's colour from the Markers category as work progresses.</Tip>
