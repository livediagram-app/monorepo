...
export const metadata = helpMetadata({ title: "Auto Layout", description: "Recompute a diagram's positions from the arrow graph, laying connected shapes into clean layers with Tidy Up.", path: "/help/features/layout-cleanup/auto-layout/" });

<ArticleLayout title="Auto Layout" description="Let livediagram compute the layout for you from the connections between shapes." categoryTitle="Features" categorySlug="features" parentArticle={{ title: "Layout Cleanup", slug: "layout-cleanup" }} subArticles={getSubArticles("layout-cleanup").filter((a) => a.slug !== "auto-layout")}>
...
- It finishes with a **grid snap** (the same one [Auto-Align](/help/features/layout-cleanup/auto-align/) runs), so the tidied output is also grid-aligned.
...
<Note>Auto Layout is deterministic and works offline, available in every deployment. It is separate from the AI "Clean" assistance, which is the optional, key-gated path.</Note>
