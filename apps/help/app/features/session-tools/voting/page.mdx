...
export const metadata = helpMetadata({ title: "Dot Voting", description: "Give each participant a budget of dots to prioritise elements, with live tallies and a results reveal.", path: "/help/features/session-tools/voting/" });

<ArticleLayout title="Dot Voting" description="Run a live dot-voting round on the canvas to prioritise as a group." categoryTitle="Features" categorySlug="features" parentArticle={{ title: "Session Tools", slug: "session-tools" }} subArticles={getSubArticles("session-tools").filter((a) => a.slug !== "voting")}>
...
<Step n={1}>Set the **dots per person** with the stepper, then choose **Start vote**. A banner tells each participant how many dots they have left.</Step>

<Step n={2}>While the vote is open, participants click the elements they want to back. A live "N cast" readout tracks the total.</Step>

<Step n={3}>Choose **End vote** to close casting (the tallies stay put), then **Show results** to reveal the winners. **Clear** ends the session.</Step>
...
<Tip>A common rule of thumb is roughly one dot per three or four options. Set the stepper before you start the vote, then leave it: keeping budgets small forces real choices.</Tip>
