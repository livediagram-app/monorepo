...
export const metadata = helpMetadata({ title: "Share Passwords", description: "Add an optional password to a diagram so anyone opening a share link must enter it first.", path: "/help/features/sharing/share-passwords/" });

<ArticleLayout title="Share Passwords" description="Gate every share link to a diagram behind a single password." categoryTitle="Features" categorySlug="features" parentArticle={{ title: "Sharing and Embeds", slug: "sharing" }} subArticles={getSubArticles("sharing").filter((a) => a.slug !== "share-passwords")}>
...
<Step n={3}>Type a password and choose **Save**. To turn it off again, clear the field or choose **Remove**.</Step>
...
<Tip>Pair a password with a [share link expiry](/help/features/sharing/share-link-expiry/) for a link that both needs a secret and stops working after a set time.</Tip>
