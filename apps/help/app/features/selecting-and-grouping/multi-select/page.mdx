...
export const metadata = helpMetadata({
  ...
  path: "/help/features/selecting-and-grouping/multi-select/",
});
...
However you gathered it, a multi-selection behaves as one. Drag any part of it and the whole set moves together, keeping its relative spacing. You can copy and paste the selection, delete it, drop it into a [group](/help/features/selecting-and-grouping/groups/), or open the right-click menu to restyle the lot at once. The [format painter](/help/features/selecting-and-grouping/using-the-format-painter/) is happy to paint a look across an entire multi-selection too.

<Tip>Clicking an empty patch of canvas clears the current selection, so you always have a quick way to start fresh before drawing a new marquee.</Tip>
