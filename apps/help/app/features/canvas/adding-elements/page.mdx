...
export const metadata = helpMetadata({
  ...
  path: "/help/features/canvas/adding-elements/",
});
...
<ArticleLayout
  ...
  subArticles={getSubArticles("canvas").filter((a) => a.slug !== "adding-elements")}
>
...
<Step n={2}>Tap an empty spot on the canvas to drop the element at its natural size on that point.</Step>
<Step n={3}>Or, instead of a single tap, drag out a box and release to create the element at that size.</Step>
...
<Tip>Icons can be added to a shape rather than as standalone elements: drag an icon tile onto a shape, or click an icon while a shape is selected, and it attaches beside that shape's label.</Tip>
