...
export const metadata = helpMetadata({ title: "Tabs and Tab Folders", description: "A diagram in livediagram holds many tabs, each its own canvas. Group related tabs into collapsible folders and keep large diagrams organised.", path: "/help/features/tabs/" });

<ArticleLayout title="Tabs and Tab Folders" description="One diagram, many canvases. Tabs let you split a project across several boards, and tab folders keep that list tidy as it grows." categoryTitle="Features" categorySlug="features" subArticles={getSubArticles("tabs")}>
...
<FeatureGrid>
  <Feature slug="tabs" title="Tab Folders">Group related tabs under a named, collapsible folder so a long tab bar stays readable.</Feature>
  <Feature slug="tabs" title="Linking Across Tabs">Link an element to another tab, or to a specific element on it, for clean drill-downs.</Feature>
</FeatureGrid>

<Tip>Tabs can be reused across diagrams behind the scenes, so a shared reference board such as a glossary can live in more than one diagram at once. Editing it in one place updates it everywhere.</Tip>
