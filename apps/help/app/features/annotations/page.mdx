...
export const metadata = helpMetadata({ title: "Annotations", description: "Pin a small themed marker to the canvas that holds a note: hover to read it, click to edit it.", path: "/help/features/annotations/" });

<ArticleLayout title="Annotations" description="A lightweight way to attach an explanatory note to a spot on the canvas without cluttering it." categoryTitle="Features" categorySlug="features" subArticles={getSubArticles("annotations")}>
...
<Step n={2}>Click **Annotation**. A new marker drops at the centre of the viewport, already selected, in one undoable step.</Step>

<Step n={3}>It inherits the active theme's colours, so it matches your diagram out of the box.</Step>
...
<Tip>Use annotations for review notes and caveats you want available on demand without crowding the diagram. They keep a consistent size, so a dozen of them still look neat.</Tip>
