...
export const metadata = helpMetadata({ title: "Roles and Invites", description: "How team roles work and the two ways to bring people into a team: by email address or with a shareable invite link.", path: "/help/features/teams/roles-and-invites/" });

<ArticleLayout title="Roles and Invites" description="Admin and Member roles, plus the two ways to bring people into a team." categoryTitle="Features" categorySlug="features" parentArticle={{ title: "Teams", slug: "teams" }} subArticles={getSubArticles("teams").filter((a) => a.slug !== "roles-and-invites")}>
...
<Step n={3}>Tell them out of band that you have invited them. (Email delivery is not part of this version, so the invite lands in their in-app Invites section rather than their inbox.)</Step>
...
<Tip>The invite link is a credential: anyone who has the URL can join while it is active. Turn it off once your team is assembled, and rely on per-email invites for tighter control.</Tip>
