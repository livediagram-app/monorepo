...
export const metadata = helpMetadata({ title: "Build, Clean, Ask, and Review", description: "The four AI modes in livediagram and when to reach for each one.", path: "/help/features/ai/ai-tools/" });

<ArticleLayout title="Build, Clean, Ask, and Review" description="The four AI modes, what each one does, and when to use it." categoryTitle="Features" categorySlug="features" parentArticle={{ title: "AI Assistance", slug: "ai" }} subArticles={getSubArticles("ai").filter((a) => a.slug !== "ai-tools")}>
...
<Note>Prompts are kept short by design, so think of each request as one clear instruction. If a result is not quite right, adjust the prompt and send again rather than stacking corrections into one long message.</Note>
