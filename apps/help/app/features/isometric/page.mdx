...
export const metadata = helpMetadata({ title: "Isometric View", description: "Tilt the current tab into a read-only isometric scene where each shape is extruded into a raised block.", path: "/help/features/isometric/" });

<ArticleLayout title="Isometric View" description="See your diagram as a tilted, extruded scene, ideal for architecture decks and system maps." categoryTitle="Features" categorySlug="features" subArticles={getSubArticles("isometric")}>
...
<Tip>Orbit to the angle that best shows the structure, then export with the Isometric view toggle on to capture exactly what you see.</Tip>
