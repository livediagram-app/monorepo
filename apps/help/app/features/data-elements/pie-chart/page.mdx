...
export const metadata = helpMetadata({ title: "Pie Chart", description: "Add an editable pie chart to the canvas and build it from simple label and value rows in your theme's colours.", path: "/help/features/data-elements/pie-chart/" });

<ArticleLayout title="Pie Chart" description="Split a total into slices you edit as plain label and value rows, drawn in your theme's colours." categoryTitle="Features" categorySlug="features" parentArticle={{ title: "Data and Chart Elements", slug: "data-elements" }} subArticles={getSubArticles("data-elements").filter((a) => a.slug !== "pie-chart")}>
...
<Step n={1}>Open the floating palette and switch to the **Tools** tab, then find the **Data** section.</Step>

<Step n={2}>Pick **Pie**. It drops onto the canvas with sample data so it looks finished straight away.</Step>
...
<Tip>Because slice colours default to your theme, changing the diagram's theme restyles the chart to match without any per-slice work.</Tip>
