...
export const metadata = helpMetadata({ title: "Comments", description: "Leave comments on the canvas to give feedback, ask questions, and track decisions, then resolve them once they are handled.", path: "/help/features/comments/" });

<ArticleLayout title="Comments" description="Drop a comment anywhere on the canvas to ask a question or leave feedback, then resolve it once it has been handled." categoryTitle="Features" categorySlug="features" subArticles={getSubArticles("comments")}>
...
<Tip>Keep each comment to one point. A marker that asks a single, specific question is far easier to resolve than a long thread covering five unrelated things.</Tip>
