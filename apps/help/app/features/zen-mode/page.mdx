...
export const metadata = helpMetadata({ title: "Zen Mode", description: "A distraction-free focus mode that hides the editor chrome and leaves just the canvas, for reading, sharing a screen, or thinking.", path: "/help/features/zen-mode/" });

<ArticleLayout title="Zen Mode" description="Hide every toolbar and panel to leave just the canvas, while keeping pan and zoom." categoryTitle="Features" categorySlug="features" subArticles={getSubArticles("zen-mode")}>
...
<Tip>Reach for zen mode when you share your screen on a call: the diagram fills the view with no toolbars to distract from what you are showing.</Tip>

<FeatureGrid>
<Feature slug="presentation" title="Presentation Mode">Go further and reveal a diagram one step at a time as a slideshow.</Feature>
<Feature slug="canvas" title="Canvas">The same pan, zoom, and fit you use everywhere stay available in zen mode.</Feature>
</FeatureGrid>
