...
/**
 * Compact social-share row for the header. Icon-only buttons that sit to the
 * left of the primary CTA; hidden on narrow screens so the CTA always fits.
 */
export function ShareButtons() {
  ...
  async function copyLink() {
    try {
      ...
      // Clipboard blocked (insecure context / permissions) — leave the icon as-is.
    }
  }

  const button =
    'inline-flex h-8 w-8 items-center justify-center rounded-md text-slate-400 transition hover:bg-slate-200/60 hover:text-brand-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-500';
  ...
  return (
    <div className="hidden items-center gap-0.5 sm:flex">
      ...
      <span aria-hidden="true" className="mx-1.5 h-5 w-px bg-slate-200" />
    </div>
  );
}
