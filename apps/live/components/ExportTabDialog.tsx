...
export function ExportTabDialog({ tab, diagramName, onClose }: ExportTabDialogProps) {
  ...
  return (
    ...
    >
      <div className="pointer-events-auto flex max-h-[90vh] w-[36rem] max-w-[92%] animate-fly-up-in flex-col rounded-xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/10">
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-6 pt-6 pb-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Export tab</h2>
...
function ExportCard({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={busy}
      className="flex flex-col items-start gap-1.5 rounded-lg border border-slate-200 bg-white p-3 text-left transition enabled:hover:border-brand-300 enabled:hover:bg-brand-50/40 disabled:cursor-not-allowed disabled:opacity-60"
    >
      <div className="flex h-12 w-full items-center justify-center rounded-md bg-slate-50">
