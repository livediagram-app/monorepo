...
- **Email** (optional): the api worker sends transactional + lifecycle email via Resend (`apps/api/src/email/`) — a welcome on first sign-in, week-1 / week-2 onboarding tips off the daily cron, plus team-invite and account-deleted messages. Gated on `RESEND_API_KEY`: no key, no sends, and the `email_lifecycle` table is never touched (see [spec/64](../specs/64-transactional-email.md)).
