...
Placement (for Duplicate) matches the gap to the nearest in-line neighbour so a duplicated chain keeps the same spacing as existing siblings instead of a fixed gap the user then has to nudge into line: the gap is the edge-to-edge distance to the nearest element that shares the source's row (left / right) or column (above / below), falling back to a 40px default when the source stands alone in that direction. It then reuses overlap-avoidance stepping (step further in the chosen direction until the new box clears existing elements), so each step lands on the same rhythm. The newly added element is selected afterward so the user can keep building outward; the Arrow option selects the new arrow instead. Open ring state is owned by the Canvas (not the individual plus) so the single-open rule and the toolbar dodge can be coordinated. By default the menu also opens on **hover** of the `+` (the `quickAddOnHover` preference, spec/20, on by default): pointer-enter opens it and it closes a beat after the pointer leaves both the `+` and the menu, so a run of connected elements doesn't need a click each time. A click still toggles it open in either mode; turning the preference off reverts to click-only. The `+` buttons themselves still appear only on the selected element.
