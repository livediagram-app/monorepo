...
## Retention (unused-image cleanup)

Uploaded bytes that no diagram references are dead weight in R2 + D1. A daily sweep reclaims them so an abandoned upload (dropped a placeholder, never picked a file; uploaded then deleted the element; tried a few candidates and kept one) doesn't accrue storage forever.

- **Schedule.** Folded into the api worker's existing daily `0 3 * * *` cron (see [12-activity-and-audit.md](12-activity-and-audit.md) and [22-telemetry.md](22-telemetry.md), which run their own retention sweeps off the same trigger). No new worker and no new cron entry: the reuse principle says a third retention sweep belongs alongside the first two in `apps/api/src/index.ts`'s `scheduled()` handler, not in a standalone worker that would re-declare the same D1 + R2 bindings.
- **What's eligible.** An image is deleted only when it is **both** (a) older than a **30-day** floor (`images.created_at < now - 30d`) **and** (b) referenced by **no** diagram. The age floor is the safety margin: a freshly uploaded image that hasn't been placed on the canvas yet (or is mid-edit) is never reaped out from under the user. 30 days is deliberately generous — this is a storage-hygiene sweep, not an aggressive GC.
- **"Referenced" is store-wide, not owner-scoped.** The sweep scans **every** tab body once (`SELECT data FROM tabs`), parses each, and keeps any image whose id appears in an `ImageElement.imageId`. It does not restrict to the image owner's diagrams, because a shared tab ([spec/17](17-tab-diagram-many-to-many.md)) can place an image inside another owner's diagram. An unparseable or orphaned tab is treated conservatively as a live reference (errs toward keeping bytes, never toward deleting a referenced one).
- **Deletion order.** R2 object first, then the D1 row (matching `DELETE /api/images/:id`). If the D1 delete fails after the R2 delete, the next sweep re-attempts both (R2 delete is idempotent), so the pair is eventually consistent. Deleting R2 first means a partial failure never strands an R2 object that no D1 candidate would ever re-surface. R2 deletes are chunked (≤1000 keys/call) and the matching D1 row deletes ride the same chunk as one `DB.batch`.
- **Self-host.** A no-op when the worker has no `IMAGES` (R2) binding — a deployment without image storage has nothing to sweep. Matches the rest of this spec's R2-optional degradation.
- **Observability.** Logs `image sweep: deleted N images older than <cutoff>` via `console.log` (visible in `wrangler tail`), the same as the change_log / events sweeps. A zero is the normal case on most days.
- **Decision (the pure part) is unit-tested.** The "which candidates are unused" decision is factored into a pure `unusedImageIds(candidateIds, tabBodies)` helper so the reference-scan logic has a test surface without a live D1 / R2 binding (the delete loop itself needs one). Same split the change_log / events sweeps and `normalizeReorderEntry` use.
