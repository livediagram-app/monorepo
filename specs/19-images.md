...
- **No EXIF stripping in v1.** Documented limitation, called out below in [Out of scope](#out-of-scope).
...
| `image/jpeg`                             | ✓       | Phone photos + any user export from another tool. Universal browser support in `<img>`. EXIF stripping is a future cycle (called out below in [Out of scope](#out-of-scope)).                                                                                                                                                                                                                        |
...
- **EXIF stripping.** Uploaded JPEGs may carry location metadata. Documented as a known limitation; a future cycle adds server-side stripping via `image/jpeg`-aware decoding (likely WASM `image-rs` in the Worker).
