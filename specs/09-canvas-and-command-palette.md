...
- **Bubble map** — a central topic circle ringed by six descriptive bubbles on a clean (blank) canvas, spokes anchored inward via `bestAnchorTowards`.
...
`TemplateDescriptor.extra?: boolean` is retained as catalogue metadata (10 default + 16 extra, pinned by `templates.test.ts`), but the picker no longer gates behind a "Show more" toggle: it browses by category (overview → drill-in, plus flat search) in a height-capped scroll area (see "Templates section" above).
