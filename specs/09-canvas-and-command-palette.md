...
A dedicated palette category **after Tools**, holding ready-made **composites** that beautify a diagram. The guiding principle: a component is **not a new element type** — each is assembled from existing primitives sharing one `groupId` (built by `create*` factories in `packages/diagram/src/factories.ts`, dropped via `addBoxedGroup`), so it moves / locks / copies as a unit yet stays fully editable and **ungroupable**, and inherits theming, resize, fonts, format-painter, copy/paste, and export for free (touching none of the per-`type` switches a bespoke element would). All follow the **active tab theme**: the themeable surfaces are filled with the theme accent (`elementStroke`, falling back to brand sky on the Basic theme — mapped in `useElementCreation` so the diagram package stays theme-agnostic), with white text for contrast.

- **Banner** — an accent-filled rounded bar with a bold **title** and a muted **subtitle**, to head a diagram (`createBanner`: a `square` shape + two `text`s). No image; offered on every viewport.
- **Hero** — a large **image** with a title + supporting line over a **theme-tinted overlay** (`createHero`: image + accent overlay at half opacity + title + body). The overlay both follows the theme and keeps the white text legible.
- **Header** — a website-style bar with a circular **avatar**, a brand/logo **title**, and a row of nav **links** (`createHeader`: accent bar + avatar + title + three link texts).
- **Avatar** — a single **circular image** (`createAvatar`: a square, aspect-locked image with a `full` corner radius + `cover` fit). Reusable standalone; also the Header's left element.

**Images + the picker.** Hero / Header / Avatar carry an image, so on drop they open the image picker on their image element (the Hero's overlay would otherwise cover the empty placeholder's upload affordance). Because they need image upload, the three are **hidden when image upload is unavailable** (no R2 / view-role — same gate as the Image tool); the Banner always shows. A new `BorderRadius` value **`full`** (clamped to 50% → a circle on a square, a pill on a rectangle) backs the circular avatar, and a new `ImageElement.objectFit` (`'cover' | 'contain'`, default `contain`) lets the hero/avatar fill their box rather than letterbox. `ImageElementView` now honours both the element's `borderRadius` and `objectFit`.

### Placement on add

The **Icon**, **Table**, **Annotation**, and the **Components** (Banner / Hero / Header / Avatar) buttons place the new element at the **centre of the visible canvas viewport** (accounting for pan), auto-selected (a component selects its group). The draw-capable buttons (shape / text / sticky / image / arrow) instead arm the combined tap-or-drag gesture — see [Adding elements](#adding-elements--tap-to-drop-or-drag-to-draw) — where a tap drops it at the tap point and a drag sizes it.
