...
The active tab carries a **`⋯` ellipsis button** to the right of its name. Clicking opens a small floating menu (right-clicking any tab opens the same menu, suppressing the browser's default context menu and switching to that tab first if it isn't active so the menu's actions operate on the tab the user pointed at). A **quick-action toolbar** (Rename / Duplicate / Lock / Delete) sits at the top; the verbose actions group into collapsible categories, banded with separators like the element context menu (rows render flush, a `MenuGroupSeparator` rules between bands): **Organise** / **Content**, then **Look & Feel** (Change Theme / Change Canvas / Auto-align) / **Font**, then **Session Tools**. The same menu — scoped to the active tab — is what the **empty-canvas right-click** opens; the **Look & Feel** + **Font** bands are present at every entry point (canvas right-click and the active tab's ellipsis), so the tab and canvas menus are identical. (The empty-canvas right-click used to open a separate canvas-only menu; it now shares this one so a right-click manages the tab as well as the canvas.) Actions:
