...
- **Theme** — a 3-column grid of preset themes. Picking a theme writes `theme: ThemeId` onto the tab AND updates the tab's `backgroundColor`, `backgroundPattern`, and `patternColor`. The backdrop follows the **same preserve-customs rule as element colours** (`switchThemeBackdrop` in `apps/live/lib/themes.ts`): each field adopts the new theme's value only when it's unset or still matches the _previous_ theme's value, so a deliberately-chosen pattern (e.g. a hand-picked Graph or Isometric) survives a theme switch instead of being reset to the theme's default grid. From that point on, newly added boxed elements inherit the theme's `elementFill / elementStroke / elementText` colours (sticky notes keep their amber identity regardless). Existing elements are **not** retroactively recoloured — the user can re-apply colours per-element via the Selected Element → Colours accordion. The theme catalogue (`apps/live/lib/themes.ts`) ships **12 default themes** (Brand, Pink, Forest, Sunset, Lavender, Mono, Ocean, Sky, Midnight, Cream, Rose, Sand) plus **9 extras behind a "Show more themes" toggle** (Olive, Indigo, Pine, Steel, Mocha, Charcoal, and the three multi-colour themes Rainbow, Pastel, Tropical — see [spec/29](29-multicolour-themes.md)). The opt-in is descriptor-driven (`extra?: boolean` on each entry), so adding more is one line in the catalogue with no UI plumbing; the in-editor Theme accordion auto-expands when the active themeId is an extra so the user always sees the active swatch. The **welcome / template picker** instead groups themes into colour-temperament **category sections** (Cool / Warm / Neutral / Multi-colour — `THEME_CATEGORIES` + `themeCategory` in `apps/live/lib/themes.ts`) inside a height-capped scroll area, mirroring how it groups templates (the `extra` flag is retained as catalogue metadata there, not used for gating). Most themes paint every element one colour; the **multi-colour** themes carry a palette and tint each branch of the diagram's hierarchy a different hue ([spec/29](29-multicolour-themes.md)). A theme card previews its colour(s) via a shared `ThemeSwatch` — one dot for single-colour themes, a row of stripes for multi-colour ones.
...
Below the welcome section, the template cards. They're grouped into **titled category sections** (Diagrams / Planning / Design / Technical — `TEMPLATE_CATEGORIES` + `templateCategory` in `apps/live/lib/templates.ts`, rendered in that order, empties skipped) inside a single **height-capped scroll area** (`max-h-[19rem]`), so the full catalogue stays one tidy bounded block instead of stretching the modal as it grows. Within each section the cards sit in a 4-column responsive grid (2-col on narrow viewports). One card is always selected (defaults to **Blank**). The catalogue (`apps/live/lib/templates.ts`, pinned by `templates.test.ts` so spec drift surfaces as a test failure) ships **24 templates** (the `extra` flag, 8 default + 16 extra, is retained as catalogue metadata; the picker now scrolls + groups rather than gating behind a "Show more" toggle):
...
- **Prioritization matrix** _(Planning)_: an Impact vs Effort 2×2 — Quick wins / Major projects / Fill-ins / Time sinks — with quadrant tints + role glyphs (like SWOT) and two axis labels (Effort →, Impact ↑). A cheap variant of the SWOT builder.

`TemplateDescriptor.extra?: boolean` is retained as catalogue metadata (8 default + 16 extra, pinned by `templates.test.ts`), but the picker no longer gates behind a "Show more" toggle: it renders all templates as category sections in a height-capped scroll area (see "Templates section" above).
