...
Below it, five top-level **view tabs** switch between ways of reading the single `GET /api/telemetry/summary` payload. On mobile the tab row **wraps** rather than overflowing a single line; Raw sits last (far right):
...
- **Look & Feel** — which visual presets get used, most to least: ranked lists of templates (`Template·Used`), built-in themes (`Theme·Changed`), and canvas patterns (`Canvas·Changed`), each with first/last tagged Most / Least used, plus the custom-theme builder's lifecycle (created / applied / edited / deleted).
- **Palette** — what people reach for in the creation palette, most to least, as three ranked lists: **Shapes** and **Components** (both `Element·Added`, split by the pre-built component catalogue — Banner, Hero, Header, Callout, StatRow, ProcessSteps, Avatar — vs everything else) and **Tools** (`Canvas·Used`: Laser, Spotlight, Eraser, FormatPainter, Isometric). Reuses the shared `RankCard` with Look & Feel. (Pointer-tool picks like select/hand are not yet in the event vocabulary, so they don't appear here.)
