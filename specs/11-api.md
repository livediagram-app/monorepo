...
| Method | Path           | Auth       | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------ | -------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| POST   | `/api/migrate` | Clerk only | Body `{ guestOwnerId }`. Reassigns every `diagrams.owner_id` + `folders.owner_id` + `shared_with.owner_id` + `user_preferences.owner_id` row from the guest id to the JWT's `sub`. `shared_with` and `user_preferences` use INSERT OR IGNORE + DELETE because a PK conflict may already exist (the same share re-accepted under both ids; or an existing prefs row from another device the user signed in from first). No `X-Owner-Id` fallback. See [spec/04](04-auth-and-guest-access.md). |
