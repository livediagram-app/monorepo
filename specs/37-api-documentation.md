...
- **Route manifest** — `apps/api/src/openapi/manifest.ts`: an array, one entry per endpoint, each `{ method, path (`/api/...`with`{param}` templates), summary, auth: 'public' | 'guest' | 'clerk' | 'either', requestSchema?: <DTO name>, responseSchema?: <DTO name>, statuses: number[] }`. The manifest is plain data referencing DTO names from `@livediagram/api-schema`; it is the single declaration of the surface.
