...
> (`apps/help`, the **Developers** category — see [spec/55](55-help-app.md)).
...
- **`manifest.ts`** — `ROUTE_MANIFEST`, one `RouteSpec` per endpoint (method,
  ...
  (`{ diagrams: DiagramSummary[] }` etc.) that aren't named DTOs.
...
realtime WebSocket op protocol stays in [spec/11](11-api.md) prose, as planned;
