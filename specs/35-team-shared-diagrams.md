...
- **Sidebar**: the personal "My Work" tree and each team in the Teams section render as the same collapsible folder tree (a team expands to reveal its folders). There is no separate "All diagrams" row — My Work lists Unsorted and the root folders directly. The in-editor floating Explorer panel's Teams accordion and its Current Diagram section give a team-library row a per-row **Delete** only when the viewer is the diagram's owner (the api restricts diagram `DELETE` to the owner, so non-owners stay open-only). Other team-row management (move, "Remove from team") stays on the full /explorer + team pages. Because team rows aren't in the personal list the panel can't optimistically prune, the panel hides a just-deleted team row locally until the team-library sweep re-fetches.
